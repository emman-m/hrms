...
<div id="chart-tardiness" class="chart-lg">
</div>

<script>
    document.addEventListener("DOMContentLoaded", function () {
        const departmentRates = <?= json_encode($tardiness['department_rates'] ?? []) ?>;
        const series = [];
        const colors = [
        ...
        if (Object.keys(departmentRates).length > 0) {
            Object.entries(departmentRates).forEach(([dept, data], index) => {
                ...
                const fullMonthData = {};
                Object.keys(<?= json_encode($tardiness['rates']) ?>).forEach(date => {
        ...
        window.ApexCharts &&
            new ApexCharts(document.getElementById("chart-tardiness"), {
                ...
                noData: {
                    text: 'No tardiness data available',
                    align: 'center',
